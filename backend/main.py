...
def load_models():
    ...
    for base in MODEL_DIR_CANDIDATES:
        try:
            ...
            # Try multiple model artifact names
            model = (
                _find_and_load(os.path.join(base, "rf_model.joblib"))
                or _find_and_load(os.path.join(base, "rf_ueba_net.joblib"))
                or _find_and_load(os.path.join(os.path.dirname(base), "model.pkl"))
                or _find_and_load(os.path.join(base, "model.pkl"))
            )
